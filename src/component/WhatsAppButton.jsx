  ...
  return (
    ...
    >
        ...
        <>
          ...
          >
            {tipIndex === 0 ? (
              ...
              <>
                <Info size={12} /> Consultation Fees Apply
              </>
            )}
          </m.div>
        </>
      ...
      <m.a
        ...
        whileTap={{ scale: 0.9 }}
        className={`relative flex h-14 w-14 items-center justify-center rounded-full shadow-2xl transition-colors duration-700 ${
          isBusinessHours ? "bg-[#25D366]" : "bg-[#101826]"
        }`}
      >
        {/* Pulsing Aura */}
        {isBusinessHours && (
        ...
        {!hasInteracted && (
          <m.span
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            className="absolute -right-1 -top-1 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-white ring-4 ring-[#FAF8F3] dark:ring-[#101826]"
          >
            1
          </m.span>
        )}
